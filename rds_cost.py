...
def rds_cost():
    ...
    print (datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S') + ': ' + 'RDS Cost Calculation(For 24 Hours) Started')
    ...
    for db_instance in db_instances:
        ...
        if engine == 'postgres':
            ...
            print datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S') + ': ' + 'Unknown DB Engine Detected. It must be other than MySQL, Aurora, MariaDB and PostgreSQL. ARN:' + arn + ' Engine: ' + engine
            continue
    ...
    print 'Total Cost(For 24 Hours) : $' + str(total_instance_cost)
    end = time.time()
    print datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S') + ': ' + 'RDS Cost Calculation Ended. Total Execution Time is ' + str(end - start) + ' Seconds'
